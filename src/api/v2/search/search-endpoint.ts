    ...
    doFulltextSearch(searchTerm: string, ontologyCache: OntologyCache, listNodeCache: ListNodeCache, offset = 0, params?: IFulltextSearchParams): Observable<ReadResource[] | ApiResponseError> {
        ...
        return this.httpGet("/search/" + encodeURIComponent(searchTerm) + SearchEndpoint.encodeFulltextParams(offset, params)).pipe(
            mergeMap((ajaxResponse: AjaxResponse) => {
                ...
                return ResourcesConversionUtil.createReadResourceSequence(jsonldobj, ontologyCache, listNodeCache, KnoraApiConfig.jsonConvert);
            }),
            catchError(error => {
    ...
    doFulltextSearchCountQuery(searchTerm: string, offset = 0, params?: IFulltextSearchParams): Observable<CountQueryResponse | ApiResponseError> {
        ...
        return this.httpGet("/search/count/" + encodeURIComponent(searchTerm) + SearchEndpoint.encodeFulltextParams(offset, params)).pipe(
            mergeMap((ajaxResponse: AjaxResponse) => {
                ...
                // console.log(JSON.stringify(jsonldobj));
                return ResourcesConversionUtil.createCountQueryResponse(jsonldobj, KnoraApiConfig.jsonConvert);
            }),
            catchError(error => {
                return this.handleError(error);
            })
